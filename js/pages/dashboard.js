...
import { proposalService, SEED_PROPOSALS } from '../../data/proposal.js';
import { konsepService, SEED_KONSEP } from '../../data/konsep.js';
...
function formatPercentOfActive(value, total) {
  if (!total) return '0%';
 ...
 *
 * 4 kartu KPI & bar "Alur peninjauan" sudah pakai data asli, gabungan
 * proposalService + konsepService (lihat pengelompokan 3-tingkat di
 * awal fungsi ini). Top Download/View juga sudah data asli lewat
 * libraryService. Yang masih dummy tinggal: grafik "per eselon 1" &
 * "berdasarkan jenis" (ESELON_DATA/JENIS_CHART_DATA di atas), dan
 * kartu Usulan Terbaru (USULAN_TERBARU_DATA) -- TODO: ganti begitu
 * ada cara masuk-akal buat sortir/gabung proposal+konsep by tanggal
 * & agregasi per satuan kerja/jenis.
 * @param {Session} user
 */
function renderFullDashboard(user) {
  // Gabungan Proposal PL + Konsep PL, dikelompokkan jadi 3 tingkat
  // yang sama dengan "Alur peninjauan" & kartu Usulan Terbaru di
  // bawah (antrian -> proses -> selesai). Ini pengelompokan yang
  // lebih kasar dari kartu ringkasan di Monitoring (lihat
  // ORTALA_CHAIN_CARD_GROUPS di js/pages/monitoring.js buat versi
  // rincinya per tahap disposisi/reviu) -- di sini cuma buat 4
  // kartu KPI atas + bar alur, jadi cukup 3 kelompok besar:
  //   - antrianReviu: DIKIRIM (baru diterima, belum disentuh)
  //   - dalamProses : PROSES_REVIU + KOREKSI_ORTALA + KOREKSI_SATKER
  //                   + SELESAI_REVIU (masih "berjalan", termasuk
  //                   yang rantai reviu-nya sudah kelar tapi belum
  //                   diputuskan Kabiro -- belum "siap terbit")
  //   - reviuSelesai: FINAL + PENGESAHAN_SATKER + LEGISLASI +
  //                   INDEKSASI (sudah resmi disetujui/"siap terbit")
  const PROSES_STATUSES = [
    SUBMISSION_STATUS.PROSES_REVIU,
    SUBMISSION_STATUS.KOREKSI_ORTALA,
    SUBMISSION_STATUS.KOREKSI_SATKER,
    SUBMISSION_STATUS.SELESAI_REVIU
  ];
  const SELESAI_STATUSES = [
  ...
  const proposalCounts = proposalService.getStatusCounts();
  const konsepCounts = konsepService.getStatusCounts();
  const sumStatuses = (statuses) =>
    statuses.reduce((sum, key) => sum + (proposalCounts[key] ?? 0) + (konsepCounts[key] ?? 0), 0);

  const kpi = {
    total: SEED_PROPOSALS.length + SEED_KONSEP.length,
    antrianReviu: sumStatuses([SUBMISSION_STATUS.DIKIRIM]),
    dalamProses: sumStatuses(PROSES_STATUSES),
    reviuSelesai: sumStatuses(SELESAI_STATUSES)
  };
    ...
    <div class="dashboard">
      ...
      <div class="dashboard__kpi-grid">
        ...
        <section class="stat-card-lg stat-card-lg--success">
          ...
          <p class="stat-card-lg__percent">${formatPercentOfActive(kpi.reviuSelesai, totalAktif)} dari berkas aktif</p>
        </section>
        ...
        <section class="stat-card-lg stat-card-lg--warning">
          ...
          <p class="stat-card-lg__percent">${formatPercentOfActive(kpi.dalamProses, totalAktif)} dari berkas aktif</p>
        </section>
        ...
        <section class="stat-card-lg stat-card-lg--danger">
          ...
          <p class="stat-card-lg__percent">${formatPercentOfActive(kpi.antrianReviu, totalAktif)} dari berkas aktif</p>
        </section>
      </div>
